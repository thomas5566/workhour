...
def get_workhour(db: Session, workhour_id: int):
    return db.query(workhour.Workhour).order_by(text("date desc")).filter(workhour.Workhour.id == workhour_id).first()
...
def get_workhours(db: Session, user_id: int):
    return db.query(workhour.Workhour).order_by(text("date desc")).filter(workhour.Workhour.user_id == user_id).all()
...
def get_workhours_by_user_id(db: Session, user_id, skip: int = 0, limit: int = 100):
    return db.query(workhour.Workhour).order_by(text("date desc")).filter(workhour.Workhour.user_id == user_id).offset(skip).limit(limit).all()
...
def get_workhours_by_task_id(db: Session, task_id: int, skip: int = 0, limit: int = 100):
    return db.query(workhour.Workhour).order_by(text("date desc")).filter(workhour.Workhour.task_id == task_id).offset(skip).limit(limit).all()


def get_workhours_by_user_task(db: Session, user_id: int, task_id: int, skip: int = 0, limit: int = 100):
    return db.query(workhour.Workhour).order_by(text("date desc")).filter(workhour.Workhour.user_id == user_id, models.Workhour.task_id == task_id).offset(skip).limit(limit).all()
...
def delete_workhour(id: int, db: Session):
    workhour_items = db.query(workhour.Workhour).filter(workhour.Workhour.id == id)
